...
void main() async {
  // Create client with the AG-UI server base URL
  ...
  await for (final event in client.runAgent('my-agent', input)) {
    switch (event) {
      ...
      case ToolCallStartEvent(toolCallName: final name):
...
When you use `AgUiClient`, pass the AG-UI server address as `AgUiClientConfig.baseUrl`:
...
When you use `HttpAgent`, the server URL is still configured on the client, and the agent itself only needs an `endpoint`:
...
You can also pass an absolute URL as the `endpoint` when needed:

```dart
final agent = HttpAgent(
  HttpAgentConfig(
    client: client,
    endpoint: 'https://api.example.com/agentic_chat',
  ),
);
```
